...
namespace XQuant {
	...
	std::wstring StringHelper::utf8ToWideChar(const std::string& utf8)
	{
		std::wstring ret;
		if (!utf8.empty())
		{
			int nNum = MultiByteToWideChar(CP_UTF8, 0, utf8.c_str(), -1, nullptr, 0);
			if (nNum)
			{
				WCHAR* wideCharString = new WCHAR[nNum + 1];
				wideCharString[0] = 0;

				nNum = MultiByteToWideChar(CP_UTF8, 0, utf8.c_str(), -1, wideCharString, nNum + 1);
	...
	std::string StringHelper::wideCharToUtf8(const std::wstring& wideChar)
	{
		std::string ret;
		if (!wideChar.empty())
		{
			int nNum = WideCharToMultiByte(CP_UTF8, 0, wideChar.c_str(), -1, nullptr, 0, nullptr, FALSE);
			if (nNum)
			{
				char* utf8String = new char[nNum + 1];
				utf8String[0] = 0;

				nNum = WideCharToMultiByte(CP_UTF8, 0, wideChar.c_str(), -1, utf8String, nNum + 1, nullptr, FALSE);
	...
	std::string StringHelper::utf8ToMultiByte(const std::string& utf8)
	{
		std::string ret;
		if (!utf8.empty())
		{
			std::wstring strWideChar = utf8ToWideChar(utf8);
